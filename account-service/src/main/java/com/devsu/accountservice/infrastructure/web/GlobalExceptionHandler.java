...
public class GlobalExceptionHandler {
  ...
  /** Returns a 404 response for missing accounts or movements. */
  @ExceptionHandler({CuentaNoEncontradaException.class, MovimientoNoEncontradoException.class})
  ResponseEntity<ApiResponse<Void>> notFound(RuntimeException exception) {
    return ResponseEntity.status(HttpStatus.NOT_FOUND)
        .body(ApiResponse.error(exception.getMessage()));
  }

  /** Returns a 409 response for duplicated account data. */
  ...
  /** Returns a 400 response for insufficient funds. */
  @ExceptionHandler(SaldoNoDisponibleException.class)
  ResponseEntity<ApiResponse<Void>> insufficientFunds(RuntimeException exception) {
